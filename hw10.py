...
Original file is located at
    https://colab.research.google.com/drive/1sQGy1zZeG9AcTQsJYDqk4Fdd6orHDuIJ
...
def ifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=30):
...
modelA = ptcv_get_model('resnet110_cifar10', pretrained=True).to(device)
modelB = ptcv_get_model('sepreresnet272bn_cifar10', pretrained=True).to(device)
modelC = ptcv_get_model('sepreresnet110_cifar10', pretrained=True).to(device)
modelD = ptcv_get_model('diaresnet110_cifar10', pretrained=True).to(device)
modelE = ptcv_get_model('densenet100_k12_bc_cifar10', pretrained=True).to(device)

# Create ensemble model
model = ensemble_Model(modelA, modelB, modelC, modelD, modelE)
